...
function getRecords($options) {
  ...
  // Get records
  list($rows, $totalRecords) = _getRecords_loadResults($query, $countQuery, $options, $schema);
...
//
function _getRecords_getCountQuery($options, $schema) {
  global $VIEWER_NAME, $TABLE_PREFIX;
...
//
function _getRecords_loadResults($query, $countQuery, $options, $schema) {
  ...
  foreach ($_REQUEST as $name => $values) {
    ...
    foreach ($fieldnames as $fieldnameString) {
      foreach ($values as $value) {
        ...
        if ($isDateField && strlen($valueAsNumberOnly) == 8) { // add time to dates without it
