...
function updateServerChangeLog($forceUpdate = false) {
  global $SETTINGS;
...
function serverInfo_contentDeliveryNetwork() {
  $name     = "";
...
function serverInfo_operatingSystem() {
  $name     = "";
  $moreInfo = ""; // show as title="" hover text
  
  // Future: Check for /proc/version
...
function serverInfo_serverName() {
  $name     = "";
  $moreInfo = ""; // show as title="" hover text
...
function serverInfo_serverAddr() {
  $name     = "";
  $moreInfo = ""; // show as title="" hover text
  ...
  if (isWindows()) {
    ...
    if (preg_match("/ /", $hostname_I)) { // spaces separate multiple IPs
...
function serverInfo_webServer_controlPanel() {
  $name     = "";
  $moreInfo = ""; // show as title="" hover text
  $links    = ""; 
  ...
  if ($wampServerConfigPath) {
    ...
    $links .= "(<a href='http://www.wampserver.com/en/'>vendor</a>, <a href='http://wampserver.aviatechno.net/?lang=en'>updates</a>)";
  }
...
function serverInfo_databaseServer() {
  $name     = "";
  $moreInfo = ""; // show as title="" hover text
  ...
  list($version_comment, $version, $basedir, $datadir) = mysql_get_query($query, true);
  ...
  $versionNumeric = preg_replace("/[^0-9\.].*/", "", $version); // remove first non-numberic char to end of string.  eg: 10.5.8-MariaDB to 10.5.8
...
function serverInfo_databaseConnection() {
  $name     = "";
  $moreInfo = ""; // show as title="" hover text
