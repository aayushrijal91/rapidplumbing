...
function __serverInfo_operatingSystem() {
  ...
  if (isWindows())  { $osNameHTML .= " (<a href='?menu=admin&action=ver'>ver</a>, <a href='?menu=admin&action=systeminfo'>systeminfo</a>)"; }
  if (!isWindows()) { $osNameHTML .= " (<a href='?menu=admin&action=releases'>release</a>)"; }
  $heading = $osNameHTML;
  ...
  if     ($percentFull >= 90) { $barStyle = "background-color: #dc3545;"; }
  elseif ($percentFull >= 80) { $barStyle = "background-color: #ffc107; color: #000;"; }
  else                        { $barStyle = ""; }
  
  if ($totalBytes)  { $diskSpaceStats  = sprintf(t('%1$s free of %2$s'), $freeBytesFormatted, $totalBytesFormatted); }
  else              { $diskSpaceStats  = t("Unavailable"); } // for servers that return 0 and "Warning: Value too large for defined data type" on big ints
  if (!isWindows()) { $diskSpaceStats .= " (<a href='?menu=admin&action=du' target='_blank'>largest dirs</a>)"; }
