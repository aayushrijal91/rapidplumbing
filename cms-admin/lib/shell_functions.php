 ...
 * @param string|null $exitCode The exit status code. 0=Success, >0=Error, Null=Exit codes not available. Note that windows and linux return different codes on failure
...
function shellCommand(string $command, ?int &$exitCode = null, ?string &$functionUsed = null): ?string {
  ...
  if (strpos($command, "\0") !== false) { dieAsCaller(__FUNCTION__. "() \$command argument cannot contain any null bytes!"); }
