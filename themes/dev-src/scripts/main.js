...
$(document).on("submit", "#become_a_member_form", function (e) {
    e.preventDefault();
        ...
        grecaptcha.ready(function () {
            grecaptcha.execute(recaptchaVal, {
                ...
                .then(function (token) {
                    ...
                    $.ajax({
                        url: SITE_URL + '/ajax/',
                        data: $("#become_a_member_form").serialize(),
                        method: 'POST',
                        success: function (response) {
                            response = response.trim();
                            if (response == "success") {
                                $("#become_a_member_form")[0].reset();
                                $(".submit_btn").removeAttr('disabled').val("SUBMIT");
                                window.location.href = SITE_URL + '/thank-you/';
                            } else if (response == "fail") {
                                alert("Failed, Something went wrong.")
...
$(document).on("submit", "#second_opinion_form", function (e) {
    e.preventDefault();
        ...
        grecaptcha.ready(function () {
            grecaptcha.execute(recaptchaVal, {
                ...
                .then(function (token) {
                    ...
                    $.ajax({
                        url: SITE_URL + '/ajax/',
                        data: $("#second_opinion_form").serialize(),
                        method: 'POST',
                        success: function (response) {
                            response = response.trim();
                            if (response == "success") {
                                $("#second_opinion_form")[0].reset();
                                $(".submit_btn").removeAttr('disabled').val("SUBMIT");
                                window.location.href = SITE_URL + '/thank-you/';
                            } else if (response == "fail") {
                                alert("Failed, Something went wrong.")
